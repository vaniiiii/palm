...
include "./legion-kyc-regex.circom";

template PalmLegionVerifier(maxHeadersLength, maxBodyLength, n, k) {
    ...
    component legionRegex = LegionKycRegex(maxBodyLength);
    legionRegex.msg <== emailBody;
    legionRegex.out === 1;
...
// maxHeadersLength=1408: Legion's DKIM-canonicalized headers are ~1024 bytes, needs headroom.
// maxBodyLength=4096: body after preselector is ~3392 bytes, 4096 gives headroom.
component main { public [ address ] } = PalmLegionVerifier(1408, 4096, 121, 17);
