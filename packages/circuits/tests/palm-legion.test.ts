...
import { generatePalmLegionInputs } from "../helpers";
...
describe("PalmLegionVerifier", function () {
  ...
  beforeAll(async () => {
    rawEmail = fs.readFileSync(
      path.join(__dirname, "./emls/legion-test.eml"),
      "utf8"
    );

    circuit = await wasm_tester(
      path.join(__dirname, "../circuits/palm-legion.circom"),
      {
        recompile: true,
        output: path.join(__dirname, "../build/palm-legion"),
        include: [
  ...
  it("should verify legion KYC email", async function () {
    const inputs = await generatePalmLegionInputs(rawEmail, ethAddress);
